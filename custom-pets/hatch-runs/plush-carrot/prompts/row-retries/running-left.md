Create Codex pet row `running-left` for `plush-carrot`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact plush carrot mascot: round soft orange body, green leafy top, cute simple face, small arms and feet when needed, cozy desk companion personality.. Preserve silhouette, face, palette, material, proportions, markings, and props.
