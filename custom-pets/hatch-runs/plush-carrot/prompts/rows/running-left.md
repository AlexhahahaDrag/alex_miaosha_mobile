Create one horizontal animation strip for Codex pet `plush-carrot`, state `running-left`.
...
Identity: same pet in every frame: A compact plush carrot mascot: round soft orange body, green leafy top, cute simple face, small arms and feet when needed, cozy desk companion personality.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `plush`: Soft plush toy mascot with rounded stitched forms, fuzzy fabric feel, simple sewn details, and readable toy-like proportions. User style notes: Codex digital pet spritesheet, plush toy texture simplified for tiny pet readability, thick clean outline, consistent face and colors, no text or UI..
