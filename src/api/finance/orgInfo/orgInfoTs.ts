...
export function getOrgInfoPage(params: any, pageNo: number | null | undefined, pageSize : number | null| undefined): Promise<any> {
  let url = baseService.finance + baseOrgInfo + OrgInfoUrl.page + '?pageNum=' + (pageNo ? pageNo : 1) + '&pageSize=' + (pageSize ? pageSize : 10);
  return postData(url, params);
}

export function getOrgInfoDetail(id: number): Promise<any> {
  return getDataOne(baseService.finance + baseOrgInfo + OrgInfoUrl.url + '?id=' + id);
}
