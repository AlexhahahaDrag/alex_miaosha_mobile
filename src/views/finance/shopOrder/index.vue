      ...
      <van-cell-group>
        <van-swipe-cell v-for='(item, index) in dataSource' :before-close='beforeClose' :key="index">
          <van-cell :title="item.id" :key='index' is-link
            :to='{ path: "/finance/shopOrder/shopOrderDetail", query: { id: item.id } }'>
            <template #label>
              <div class="iconClass">
            ...
            <template #right-icon>
              <div class='text-right'>
                ...
                <div :class="true ? 'rightDiv' : 'rightRedDiv'">
                    item.isValid+item.saleDate+item.description+item.payWay+item.saleCount+;
                </div>
              </div>
            </template>
          </van-cell>
          <template #right>
            <van-button class='right_info' @click='delShopOrder(item.id)' square type='danger' text='删除' />
          </template>
...
const delShopOrder = (id: number): void => {
  deleteShopOrder(id + '').then((res: any) => {
    if (res?.code == '200') {
      refresh();
