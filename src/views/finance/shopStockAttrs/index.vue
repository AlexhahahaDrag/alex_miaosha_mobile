<template>
  <navBar :info='info' @clickRight='addShopStockAttrs'></navBar>
      ...
      <van-cell-group>
        <van-swipe-cell v-for='(item, index) in dataSource' :before-close='beforeClose' :key="index">
          <van-cell :title="item.id" :key='index' is-link
            :to='{ path: "/finance/shopStockAttrs/shopStockAttrsDetail", query: { id: item.id } }'>
            <template #label>
              <div class="iconClass">
            ...
            <template #right-icon>
              <div class='text-right'>
                ...
                <div :class="true ? 'rightDiv' : 'rightRedDiv'">
                    item.attrValue+item.isValid+item.description+;
                </div>
              </div>
            </template>
          </van-cell>
          <template #right>
            <van-button class='right_info' @click='delShopStockAttrs(item.id)' square type='danger' text='删除' />
          </template>
