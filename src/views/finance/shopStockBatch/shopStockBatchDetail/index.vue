<template>
    ...
    <van-form @submit="onSubmit" :rules="rulesRef" required="auto">
        <van-cell-group>
            <van-field v-model="formInfo.batchCode" name="batchCode" :label="label.batchCode + '：'"
                :placeholder="'请输入' + label.batchCode" :rules="rulesRef.batchCode" :maxlength="255" />
            <van-field v-model="formInfo.batchName" name="batchName" :label="label.batchName + '：'"
                :placeholder="'请输入' + label.batchName" :rules="rulesRef.batchName" :maxlength="255" />
