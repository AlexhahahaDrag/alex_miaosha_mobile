<template>
  <NavBar :info='info' @clickRight='addPmsAttr'></NavBar>
      ...
      <van-cell-group>
        <van-swipe-cell v-for='(item, index) in dataSource' :before-close='beforeClose' :key="index">
          <van-cell :title="item.id" :key='index' is-link
            :to='{ path: "/product/pmsAttr/pmsAttrDetail", query: { id: item.id } }'>
            <template #label>
              <div class="iconClass">
          ...
          <template #right>
            <van-button class='right_info' @click='delPmsAttr(item.id)' square type='danger' text='删除' />
          </template>
