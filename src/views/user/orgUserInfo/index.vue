<template>
  <NavBar :info='info' @clickRight='addOrgUserInfo'></NavBar>
      ...
      <van-cell-group>
        <van-swipe-cell v-for='(item, index) in dataSource' :before-close='beforeClose' :key="index">
          <van-cell :title="item.id" :key='index' is-link
            :to='{ path: "/user/orgUserInfo/orgUserInfoDetail", query: { id: item.id } }'>
            <template #label>
              <div class="iconClass">
          ...
          <template #right>
            <van-button class='right_info' @click='delOrgUserInfo(item.id)' square type='danger' text='删除' />
          </template>
...
const delOrgUserInfo = (id: number) => {
  deleteOrgUserInfo(id + '').then((res: any) => {
    if (res?.code == '200') {
      refresh();
