<template>
  <NavBar :info='info' @clickRight='addRoleInfo'></NavBar>
      ...
      <van-cell-group>
        <van-swipe-cell v-for='(item, index) in dataSource' :before-close='beforeClose' :key="index">
          <van-cell :title="item.id" :key='index' is-link
            :to='{ path: "/user/roleInfo/roleInfoDetail", query: { id: item.id } }'>
            <template #label>
              <div class="iconClass">
          ...
          <template #right>
            <van-button class='right_info' @click='delRoleInfo(item.id)' square type='danger' text='删除' />
          </template>
...
const delRoleInfo = (id: number) => {
  deleteRoleInfo(id + '').then((res: any) => {
    if (res?.code == '200') {
      refresh();
