<template>
  <NavBar :info='info' @clickRight='addRolePermissionInfo'></NavBar>
      ...
      <van-cell-group>
        <van-swipe-cell v-for='(item, index) in dataSource' :before-close='beforeClose' :key="index">
          <van-cell :title="item.id" :key='index' is-link
            :to='{ path: "/user/rolePermissionInfo/rolePermissionInfoDetail", query: { id: item.id } }'>
            <template #label>
              <div class="iconClass">
          ...
          <template #right>
            <van-button class='right_info' @click='delRolePermissionInfo(item.id)' square type='danger' text='删除' />
          </template>
...
const delRolePermissionInfo = (id: number) => {
  deleteRolePermissionInfo(id + '').then((res: any) => {
    if (res?.code == '200') {
      refresh();
